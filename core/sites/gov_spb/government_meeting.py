...
SEARCH_PAGE_URL = "https://www.gov.spb.ru/press/government_meeting/?page=%s"
PAGE_URL = "https://www.gov.spb.ru"


def parsing_gov_government_meeting(limit_date, proxy):
    articles = []
    page = 1
    i = 0
    is_ok = True
    while page < 1000:
        ...
        if len(body) == 0:
            break
        for article in body:
            print("parsing_gov_government_meeting " + str(i))
            i += 1
            try:
    ...
    try:
        ...
        if res.ok:
            ...
            try:
                ...
                pass
            article_date = dateparser.parse(soup.find("div", {"class": "source__date"}).text.strip())
