...
SEARCH_PAGE_URL = "https://saintpetersburgcb.com/events/?page=%s"
...
def parsing_events():
    wb = load_workbook(filename='Events.xlsx')
    sheet = wb['Лист1']
    ws = wb.active
    ...
    if res.ok:
        ...
        for article in articles:
            try:
                ...
                hrefs.append(href)
                try:
                    img = PAGE_URL + article.find("img").attrs.get("src")
                except Exception:
                    img = ""
                body.append(
                        {
...
def get_page(articles, article_body, attempt=0):
    try:
        ...
        if res.ok:
            ...
            site = ""
            try:
                addr = PAGE_URL + soup_all.find("div", {"class": "address"}).find("a").attrs["href"]
            except Exception:
                addr = ""
            for d in soup_all.find("div", {"class": "row item-info"}).find_all("div", {"class": "row"}):
                try:

                    if 'Сайт мероприятия' in d.find("div", {"class": "column-5"}):
            ...
            res = {"href": article_body['href'],
                   ...
                   "site": site,
                   "addr": addr,
                   "category":soup_all.find("div", {"class": "category"}).text.strip()
                   }
